...
import { RightSideNewsContainerStyle } from "../Components/style";
export const RightSideNewsContainer = () => {
  const [data, setData] = useState([]);
  const [data2, setData2] = useState([]);
  const CallData = async () => {
    let promise = await fetch(
      "https://newsapi.org/v2/everything?q=trending&apiKey=cf207e12c5944860844ef44fab09b3df"
    );
    let d = await promise.json();
    setData(d.articles);
    let promise2 = await fetch(
      "https://newsapi.org/v2/everything?q=viral&apiKey=cf207e12c5944860844ef44fab09b3df"
    );
    ...
    <RightSideNewsContainerStyle >
      <div id="Rightdiv1">
        ...
        >
          ...
          >
            <span style={{ color: "#00b1cd", fontSize: "20px" }}>[</span>
            TRENDING
            <span style={{ color: "#00b1cd", fontSize: "20px" }}>]</span>
          </div>
        ...
        {data?.map((el, i) => {
          ...
          if (el.description) {
            if (el.description[0] !== "<") {
              ...
              count++;
              return (
                <div className="RightNewsDiv"  key={el.id}>
                  <div>
      ...
      <div id="Rightdiv2">
        ...
        >
          ...
          >
            <span style={{ color: "#00b1cd", fontSize: "20px" }}>[</span>VIRAL
            VIDEOS<span style={{ color: "#00b1cd", fontSize: "20px" }}>]</span>
          </div>
